...
+ function($) {
    ...
    var initMonthes = [month + 1, month + 2, month + 3];
    ...
    for (var i = 0; i < initMonthes.length; i++) {
        if (initMonthes[i] > 12) {
            initMonthes.splice(i);
        }
    }
    var initMonthesLen = initMonthes.length;
    if (month >= 10) {
        var initYears = (function() {
            var years = new Date().getFullYear();
            var arr = [years, years + 1];
            return arr;
        })();
    } else {
        var initYears = (function() {
    ...
    var defaults = {
        ...
        value: [today.getFullYear(), formatNumber(today.getMonth() + 2), formatNumber(today.getDate())],

        onChange: function(picker, values, displayValues) {

            if (picker.cols[0].value == new Date().getFullYear()) {
                var days = getDaysByMonthAndYear(picker.cols[1].value, picker.cols[0].value);
                var moht = month + 1;
                var currentValue = picker.cols[2].value;
                var currentMonthValue = picker.cols[1].value;
                if (currentValue > days.length) currentValue = days.length;
                if (currentMonthValue < moht) currentMonthValue = moht;
                picker.cols[2].setValue(currentValue);
                picker.cols[1].setValue(currentMonthValue);

            } else {
                var days = getDaysByMonthAndYear(picker.cols[1].value, picker.cols[0].value);
                var monthNew = +3 - (12 - month);
                monthNew < 10 ? monthNew = ('0' + monthNew) : monthNew;

                var currentValue = picker.cols[2].value;
                if (currentValue > days.length) currentValue = days.length;
                picker.cols[2].setValue(currentValue);
                picker.cols[1].setValue(monthNew);
            }
        },
