<?php
class RetailAction extends ShopAction{
	...
	public function departSet(){
		...
		$id = session('sid');
		$set 	  = M('Store_list')->where('id='.$id)->find();
	...
	public	function additionalTime(){
			...
			if(IS_POST){
				...
				if($limit){
					$this->error('该日期限制已经添加过',U('Retail/additionalTime'));
				}else{
					...
					if($re){
						$this->success('添加成功',U('Retail/schedule',array('token'=>$token,'sid'=>$_SESSION['sid'])));
					}else{
						$this->error('添加失败',U('Retail/schedule',array('token'=>$token,'sid'=>$_SESSION['sid'])));
					}
				}
				...
				$store = M('Store_list')->where($where)->find();
	...
	public function dataEdit(){
		...
		if(IS_POST){
				...
				$store = M('Store_list')->where(array('id'=>$addtime['sid']))->find();
				$this->assign('store',$store);
				$this->assign('set',$addtime);
