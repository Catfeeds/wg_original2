...
<div class="welcome">欢迎使用多用户微信营销服务平台!111</div>
    <div class="memberinfo"  id="destoon_member">	
		<?php if($_SESSION['sid']==false): ?><a href="<?php echo U('Index/shoplogin');?>">登录</a>&nbsp;&nbsp;|&nbsp;&nbsp;
			<a href="<?php echo U('Index/shoplogin');?>">注册</a>
			<?php else: ?>
			&nbsp;你好,
			
			<span style="color:red">
			<?php echo (session('shopname')); ?>
			</span>
			（sid:<?php echo (session('sid')); ?>）
			<a href="<?php echo U('Home/Users/shoplogout');?>" >退出</a><?php endif; ?>	
	</div>
	</div>
    </div>
</div>
...
<?php
  ...
  <div class="contentmanage"
  	<?php if (isset($_SESSION['isQcloud'])){ ?> 
  		style="width:100%"<?php }?>>
  	<?php
 if (!isset($_SESSION['isQcloud'])){ ?>
    <div class="developer" style="border:0px">
...
<?php
 $menus=array( array( 'name'=>'分店管理', 'iconName'=>'retail', 'display'=>0, 'subs'=>array( array('name'=>'店铺设置','link'=>U('Retail/departset',array('token'=>$token)),'new'=>0,'selectedCondition'=>array('m'=>'Retail')), array('name'=>'微信商城系统','link'=>U('Store/index',array('token'=>$token)),'new'=>0,'selectedCondition'=>array('m'=>'Store')), )), ); ?>
<?php
...
<a href="<?php echo U('Retail/additionalTime',array('token'=>$token,'id'=>$_GET['id']));?>" title="添加" class="btnGrayS vm bigbtn">
<img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加日程安排</a>
...
<td><?php echo (date("Y-m-d",$hostlist["time"])); ?></td>
<td><?php echo ($storename); ?></td>
<td class="norightborder">

	<a href="<?php echo U('Retail/dataEdit',array('token'=>$token,'id'=>$hostlist['id']));?>">详情</a>

  <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Retail/delDtime',array('id'=>$hostlist['id'],'token'=>$token));?>');">删除</a>
</td>
