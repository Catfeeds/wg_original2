...
/*	public function companyLogout()
	...
	public function shoplogin(){
		...
		if (isset($_POST['verifycode2'])){
			if($verifycode != $_SESSION['loginverify']){
				$this->error('验证码错误',U('Index/shoplogin'));
			}
		}
		...
		if($res&&($pwd===$res['password'])){		
			...
			$this->success('登录成功',U('Shop/Retail/departset'));
			
		}else{
			$this->error('帐号密码错误',U('Index/shoplogin'));
		}
	}
    ...
    public function shoplogout() {
        ...
        if(session('?'.C('USER_AUTH_KEY'))) {
            session(C('USER_AUTH_KEY'),null);   
            redirect(U('User/Index/shoplogin'));
        }else {
            $this->error('已经登出！',U('Home/Index/shoplogin'));
        }
    }		
}